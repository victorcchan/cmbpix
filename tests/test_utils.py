...
def test_patches_properties():
	...
	assert np.all(np.array(patches0nside1to4) < hp.nside2npix(4)), \
		"cmbpix.patches does not return valid subindices for NSIDE 1->4"
	assert np.all(np.array(patches0nside1to512) < hp.nside2npix(512)), \
